...
for sum in range(1, 9):
    print(sum)
    if sum != 1: continue
    want_octant = False #f"octonly_all{sum:d}" #False #f"octant_all{sum:d}" # "octant_all", "octonlysmooth", "octonly", "octant", False

    redshift_s = f"/z{z_s:.3f}/galaxies{rsd_str}/" # galaxy lensing tracer (source)
    redshift_l = f"/z{z_l:.3f}/galaxies{rsd_str}/" # number tracer
    if want_octant:
        file_name_s = cat_dir+sim_name+redshift_s+f"{tracer}s_{want_octant}_catalog.asdf"
        file_name_l = cat_dir+sim_name+redshift_l+f"{tracer}s_{want_octant}_catalog.asdf"
    else:
    ...
    RA_lens_s = asdf.open(file_name_s)['data']['RA_lens']
    DEC_lens_s = asdf.open(file_name_s)['data']['DEC_lens']
