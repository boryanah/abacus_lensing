...
np.savez(f"data/NN_zl{z_l:.3f}_zs{z_s:.3f}.npz", r=np.exp(dd.meanlogr), xi=xi, err=np.sqrt(varxi), xi_mag_p=xi_mag_p, err_mag_p=np.sqrt(varxi_mag_p))
print('Time for calculating nn correlation = ', time.time()-t1)


t1 = time.time()
gg.process(cat_s)  # Takes approx 1 minute / million objects
np.savez(f"data/GG_zl{z_l:.3f}_zs{z_s:.3f}.npz", xip=gg.xip, xim=gg.xim, r=np.exp(gg.meanlogr), err=np.sqrt(gg.varxip))
